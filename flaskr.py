...
@app.route('/')
def nav():
    cur = g.db.execute('select id, name, url from nav')
    nav = [dict(id=row[0], name=row[1], url=row[2]) for row in cur.fetchall()]
    return render_template('nav.html', nav=nav)

@app.route('/analysis')
def analysis():
    cur = g.db.execute('select id, name, url, group_id, sort_id from analysis_menu')
...
def analysis_gold():
    return render_template('analysis_gold.html')
